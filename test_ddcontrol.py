...
def test_PIDTuner():
    ...
    for index in range(y.shape[0]):
        y[index] = mdl.update(u_control)
        if index < 100:
            u_control = tuner.update(-y[index], y[index], tune=False)
        elif index < 300:
            u_control = tuner.update(-y[index], y[index], tune=True)
        else:
            u_control = tuner.update(-y[index], y[index], tune=False)

    from matplotlib import pyplot
    pyplot.plot(tuner.history[:,1], '.-')
    pyplot.show()
    assert False